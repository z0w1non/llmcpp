...
* VRAM 容量に制限に起因して同時に複数のサーバーを起動することができないローカル環境に対し、統一的な方法で各サーバーを起動・終了する。
...
あとがきや次回予告は不要です。
登場人物の個性を一貫させてください。
物語はゆっくり進展させ、細部を面白く描写してください。
物語は後述する登場人物のみで構成してください。
物語は平和で痛みのないものにしてください。
物語はコメディ、スローライフ、日常系です。
...
`--create-process` が指定された場合、サーバーを起動するコマンドを非同期に実行後、`--server-host` と `--server-port` により指定された IPアドレス とポートと通信が可能になるまで待機する。
