...
## 機能
* EBNF パーサおよび再帰下降構文解析により実装された独自のマクロ機能および評価エンジンによる、プログラミング可能なプロンプト生成を提供する。
...
* ユーザーによりカスタマイズされた処理による実行時例外を含む例外発生時に、自動でスタックトレースやソースコード位置情報、その他を含むデバッグに有益な情報を、重要度に応じてフィルタリング可能な形態でログ出力する、堅牢なエラーハンドリング機構を提供する。
