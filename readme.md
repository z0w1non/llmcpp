...
### `{{head("filename", 1024}}`
指定されたテキストファイルの内容のうち、先頭から指定されたトークン数分の文字列に展開される。ファイル名は拡張子を省略して記述し、実行時に `.txt` を補完して解釈される。例えば `{{head("output", 1024)}}` のように記述して、最古の出力をプロンプトに含めることができる。

### `{{tail("filename", 1024}}`
指定されたテキストファイルの内容のうち、末尾から指定されたトークン数分の文字列に展開される。ファイル名は拡張子を省略して記述し、実行時に `.txt` を補完して解釈される。例えば `{{tail("output", 1024)}}` のように記述して、最新の出力をプロンプトに含めることができる。

### `{{json_literal("text")}}`
生改行を `\n` に置換するなど、JSON の文字列リテラルとして正しくなるようエスケープする。`{ "key": "{{json_literal(include("filename"))}}" }` などと使用する。
