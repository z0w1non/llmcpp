...
パスの区切り文字は `/` でなければならない。ファイル名は拡張子を省略して記述し、実行時に `.txt` を補完して解釈される。

### `{{include("filename")}}`
`:` の右側で指定されたテキストファイルの内容に展開される。
これによりプロンプトを複数のファイルに分割して管理することができる。

### `{{head("filename", 1024}}`
指定されたテキストファイルの内容のうち、先頭から指定されたトークン数分の文字列に展開される。
例えば `{{head("output", 1024)}}` のように記述して、最古の出力をプロンプトに含めることができる。

### `{{tail("filename", 1024}}`
指定されたテキストファイルの内容のうち、末尾から指定されたトークン数分の文字列に展開される。
例えば `{{tail("output", 1024)}}` のように記述して、最新の出力をプロンプトに含めることができる。

### `{{head_tail("filename", 512, 512}}`
指定されたテキストファイルの内容のうち、先頭と末尾それぞれから指定されたトークン数分の文字列に展開される。
例えば `{{head_tail("output", 512, 512)}}` のように記述して、最古と最新の出力をプロンプトに含めることができる。
先頭と末尾の間は `...` で表現される。

### `{{json_literal("text")}}`
生改行を `\n` に置換するなど、JSON の文字列リテラルとして正しくなるようエスケープする。
例えば `{ "key": "{{json_literal(include("filename"))}}" }` などと使用する。
