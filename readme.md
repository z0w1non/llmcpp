...
## 概要
llmcpp は、生成AIによりテキスト・画像・音声を生成する作業を支援する、ローカル環境で動作可能な CLI のフロントエンドである。
...
## 機能
* EBNF パーサおよび再帰下降構文解析により実装された、独自のマクロ機能および評価エンジンから構成されるテンプレート言語による、プログラミング可能なプロンプト生成を提供する。
...
`;` はそれ以前の式を棄却する構文であり、マクロ式全体が最後に `;` で棄却された場合、それは空文字列に展開される。
この仕様は、代入演算と同様の単純な構文でマクロ変数を宣言することを可能にし、かつそれを評価した結果が文字列として展開されることを回避することを目的として設計されている。
副作用を目的としたマクロは、基本的に文字列に展開することを期待されないため、
