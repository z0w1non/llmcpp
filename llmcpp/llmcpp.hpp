    ...
    namespace command_line
    {
        ...
        boost::program_options::options_description make_options_description(config& cfg)
        {
            ...
            const auto value
            {
                [](auto& var)
                {
                    return po::value<std::decay_t<decltype(var)>>(&var);
                }
            };
            ...
            options_description.add_options()
                ...
                ("base-path", value(cfg.base_path)->default_value("."), "base path")
                ...
                ("log-file", value(cfg.log_file)->default_value("log"), "log file path")
                ("config-file,c", value(cfg.config_file)->default_value("config.ini"), "config file path")
                ("verbose,v", po::bool_switch(&cfg.verbose)->default_value(false), "enable verbose output")
                ("number-iterations,N", value(cfg.number_iterations)->default_value(1), "number of iterations (-1 means infinity)")
                ...
                ("seed", value(cfg.seed)->default_value(-1), "seed value")
                ...
                ("png-file", value(cfg.png_file)->default_value(""), "for extract-png-parametesrs")
                ("server-executable-file", value(cfg.server_executable_file)->default_value(""), "server executable file")
                ("server-arguments", value(cfg.server_arguments), "server arguments")
                ("server-host", value(cfg.server_host)->default_value("localhost"), "server ip")
                ("server-port", value(cfg.server_port)->default_value("5000"), "server port")
                ("server-max-retries", value(cfg.server_max_retries)->default_value(60), "server max retries")
                ("server-wait-ms", value(cfg.server_wait_ms)->default_value(1000), "server wait ms")
                ("timeout-connect", value(cfg.timeout_connect)->default_value(10), "Time limit for establishing the connection (handshake completion)")
                ("timeout-request", value(cfg.timeout_request)->default_value(0), "Time limit from sending the request to completing the receipt of the response.")

                ("llm-prompt", po::value<std::string>()->default_value("")->notifier(make_unescape_string_notifier(cfg.llm.prompt)), "LLM prompt")
                ("llm-prompt-file", value(cfg.llm.prompt_file)->default_value("prompt"), "LLM prompt file path")
                ("llm-output-file", value(cfg.llm.output_file)->default_value("output"), "LLM output file path")
                ("llm-chat-file", value(cfg.llm.chat_file)->default_value(""), "LLM (input / output) chat file path")
                ...
                ("llm-paragraphs-file", value(cfg.llm.paragraphs_file)->default_value(""), "LLM paragraphs file")
                ("llm-image-file", value(cfg.llm.image_file)->default_value(""), "LLM image file")
                ("llm-host", value(cfg.llm.host)->default_value("localhost"), "LLM host")
                ("llm-port", value(cfg.llm.port)->default_value("5000"), "LLM port")
                ("llm-api-key", value(cfg.llm.api_key)->default_value(""), "LLM API key")
                ("llm-completions-target", value(cfg.llm.completions_target)->default_value(""), "LLM completions target")
                ("llm-token-count-target", value(cfg.llm.token_count_target)->default_value(""), "LLM token count target")
                ("llm-chat-completions-target", value(cfg.llm.chat_completions_target)->default_value(""), "LLM chat completions target")
                ("llm-min-completion-tokens", value(cfg.llm.min_completion_tokens)->default_value(256), "LLM min completion tokens")
                ("llm-max-completion-iterations", value(cfg.llm.max_completion_iterations)->default_value(5), "LLM max completion iterations")
                ...
                ("tg-model", value(cfg.tg.model)->default_value("", "TG model"))
                ("tg-num-best-of", value(cfg.tg.best_of)->default_value(1), "TG best of")
                ("tg-echo", po::bool_switch(&cfg.tg.echo)->default_value(false), "TG echo")
                ("tg-frequency-penalty", value(cfg.tg.frequency_penalty)->default_value(0.0), "TG frequency penalty")
                //std::map<int, double> logit_bias;
                ("tg-logprobs", value(cfg.tg.logprobs)->default_value(0.0), "TG presence penalty")
                ("tg-max-tokens", value(cfg.tg.max_tokens)->default_value(512), "TG max tokens")
                ("tg-n", value(cfg.tg.n)->default_value(1), "TG number of responses generated for the same prompt")
                ("tg-presence-penalty", value(cfg.tg.presence_penalty)->default_value(0.0), "TG presence penalty")
                ...
                ("tg-suffix", value(cfg.tg.suffix)->default_value(""), "TG suffix")
                ("tg-temperature", value(cfg.tg.temperature)->default_value(1.0), "TG temperature")
                ("tg-top-p", value(cfg.tg.top_p)->default_value(1.0), "TG top p")
                ("tg-dynatemp-low", value(cfg.tg.dynatemp_low)->default_value(0.75, "0.75"), "TG dynatemp low")
                ("tg-dynatemp-high", value(cfg.tg.dynatemp_high)->default_value(1.25, "1.25"), "TG dynatemp high")
                ("tg-dynatemp-exponent", value(cfg.tg.dynatemp_exponent)->default_value(1.0), "TG dynatemp exponent")
                ("tg-smoothing-factor", value(cfg.tg.smoothing_factor)->default_value(0.0), "TG smoothing factor")
                ("tg-smoothing-curve", value(cfg.tg.smoothing_curve)->default_value(1.0), "TG smoothing curve")
                ("tg-min-p", value(cfg.tg.min_p)->default_value(0.1, "0.1"), "TG min p")
                ("tg-top-k", value(cfg.tg.top_k)->default_value(0), "TG top k")
                ("tg-typical-p", value(cfg.tg.typical_p)->default_value(1.0), "TG typical p")
                ("tg-xtc-threshold", value(cfg.tg.xtc_threshold)->default_value(0.1, "0.1"), "TG Exclude Top Choices (XTC) threshold")
                ("tg-xtc-probability", value(cfg.tg.xtc_probability)->default_value(0.0), "TG Exclude Top Choices (XTC) probability")
                ("tg-epsilon-cutoff", value(cfg.tg.epsilon_cutoff)->default_value(0), "TG epsilon cutoff")
                ("tg-eta-cutoff", value(cfg.tg.eta_cutoff)->default_value(0), "TG eta cutoff")
                ("tg-tfs", value(cfg.tg.tfs)->default_value(1.0), "TG tfs")
                ("tg-top-a", value(cfg.tg.top_a)->default_value(0.0), "TG top a")
                ("tg-top-n-sigma", value(cfg.tg.top_n_sigma)->default_value(1.0), "TG top n sigma")
                ("tg-dry-multiplier", value(cfg.tg.dry_multiplier)->default_value(0.0), "TG DRY multiplier")
                ("tg-dry-allowed-length", value(cfg.tg.dry_allowed_length)->default_value(2), "TG DRY allowed length")
                ("tg-dry-base", value(cfg.tg.dry_base)->default_value(1.75), "TG DRY base")
                ("tg-repetition-penalty", value(cfg.tg.repetition_penalty)->default_value(1.2), "TG repetition penalty")
                ("tg-encoder-repetition-penalty", value(cfg.tg.encoder_repetition_penalty)->default_value(1.0), "TG encoder repetition penalty")
                ("tg-no-repeat-ngram-size", value(cfg.tg.no_repeat_ngram_size)->default_value(0), "TG no repeat ngram size")
                ("tg-repetition-penalty-range", value(cfg.tg.repetition_penalty_range)->default_value(0), "TG repetition penalty range")
                ("tg-penalty-alpha", value(cfg.tg.penalty_alpha)->default_value(0.9, "0.9"), "TG penalty alpha")
                ("tg-guidance-scale", value(cfg.tg.guidance_scale)->default_value(1.0), "TG guidance scale")
                ("tg-mirostat-mode", value(cfg.tg.mirostat_mode)->default_value(0), "TG mirostat mode")
                ("tg-mirostat-tau", value(cfg.tg.mirostat_tau)->default_value(5), "TG mirostat tau")
                ("tg-mirostat-eta", value(cfg.tg.mirostat_eta)->default_value(0.1, "0.1"), "TG mirostat eta")
                ("tg-prompt-lookup-num-tokens", value(cfg.tg.prompt_lookup_num_tokens)->default_value(0), "TG prompt lookup num tokens")
                ("tg-max-tokens-second", value(cfg.tg.max_tokens_second)->default_value(0), "TG max tokens second")
                ...
                ("tg-truncation-length", value(cfg.tg.truncation_length)->default_value(4096), "TG truncation length")
                ("tg-sampler-priority", value(cfg.tg.sampler_priority)->multitoken()->default_value(default_sampler_priority, boost::algorithm::join(default_sampler_priority, " ")), "TG sampler priority")
                ("tg-custom-token-bans", value(cfg.tg.custom_token_bans)->default_value(""), "TG custom token bans")
                ("tg-negative-prompt", value(cfg.tg.negative_prompt)->default_value(""), "TG negative prompt")
                ...
                ("tg-grammar-string", value(cfg.tg.grammar_string)->default_value(""), "TG grammar-string")

                ("kc-max-context-length", value(cfg.kc.max_context_length)->default_value(4096), "Maximum number of tokens to send to the model. (minimum: 1)")
                ("kc-max-length", value(cfg.kc.max_length)->default_value(512), "Number of tokens to generate. (minimum: 1)")
                ("kc-rep-pen", value(cfg.kc.rep_pen)->default_value(1.0), "Base repetition penalty value. (minimum: 1.0)")
                ("kc-rep-pen-range", value(cfg.kc.rep_pen_range)->default_value(0), "Repetition penalty range. (minimum: 0)")
                ...
                ("kc-sampler-seed", value(cfg.kc.sampler_seed)->default_value(1), "RNG seed to use for sampling. If not specified, the global RNG will be used. (minimum: 1, maximum: 999999)")
                ...
                ("kc-temperature", value(cfg.kc.temperature)->default_value(1.0), "Temperature value.")
                ("kc-tfs", value(cfg.kc.tfs)->default_value(1.0), "Tail free sampling value. (minimum: 0.0, maximum: 1.0)")
                ("kc-top-a", value(cfg.kc.top_a)->default_value(1.0), "Top-a sampling value. (minimum: 0.0)")
                ("kc-top-k", value(cfg.kc.top_k)->default_value(0.0), "Top-k sampling value. (minimum: 0.0)")
                ("kc-top-p", value(cfg.kc.top_p)->default_value(1.0), "Top-p sampling value. (minimum: 0.0, maximum: 1.0)")
                ("kc-min-p", value(cfg.kc.min_p)->default_value(0.1), "Min-p sampling value. (minimum: 0.0, maximum: 1.0)")
                ("kc-typical", value(cfg.kc.typical)->default_value(1.0), "Typical sampling value. (minimum: 0.0, maximum: 1.0)")
                ...
                ("kc-dynatemp_range", value(cfg.kc.dynatemp_range)->default_value(0.0), "If not equal to 0, uses dynamic temperature. Dynamic temperature range will be between Temp+Range and Temp-Range. If equal to 0 , uses static temperature. (default: 0, minimum: -5.0, maximum: 5.0)")
                ("kc-smoothing-factor", value(cfg.kc.smoothing_factor)->default_value(0.0), "Modifies temperature behavior. If greater than 0 uses smoothing factor. (default: 0.0, minimum: 0.0)")
                ("kc-dynatemp-exponent", value(cfg.kc.dynatemp_exponent)->default_value(1.0), "Exponent used in dynatemp. (default: 0.0)")
                ("kc-mirostat", value(cfg.kc.mirostat)->default_value(0), "KoboldCpp ONLY. Sets the mirostat mode, 0=disabled, 1=mirostat_v1, 2=mirostat_v2. (minimum: 0, maximum: 2)")
                ("kc-mirostat-tau", value(cfg.kc.mirostat_tau)->default_value(0.0), "KoboldCpp ONLY. Mirostat tau value. (minimum: 0.0)")
                ("kc-mirostat-eta", value(cfg.kc.mirostat_eta)->default_value(0.0), "KoboldCpp ONLY. Mirostat eta value. (minimum: 0.0)")
                ("kc-genkey", value(cfg.kc.genkey)->default_value(""), "KoboldCpp ONLY. A unique genkey set by the user. When checking a polled-streaming request, use this key to be able to fetch pending text even if multiuser is enabled.")
                ("kc-grammar", value(cfg.kc.grammar)->default_value(""), "KoboldCpp ONLY. A string containing the GBNF grammar to use.")
                ...
                ("kc-dry-multiplier", value(cfg.kc.dry_multiplier)->default_value(0.0), "KoboldCpp ONLY. DRY multiplier value, 0 to disable. (minimum: 0)")
                ("kc-dry-base", value(cfg.kc.dry_base)->default_value(1.75), "KoboldCpp ONLY. DRY base value. (minimum: 0)")
                ("kc-dry-allowed-length", value(cfg.kc.dry_allowed_length)->default_value(2), "KoboldCpp ONLY. DRY allowed length value. (minimum: 0)")
                ("kc-dry-penalty-last-n", value(cfg.kc.dry_penalty_last_n)->default_value(0), "KoboldCpp ONLY. DRY last n tokens penalized value. (minimum: 0)")
                ...
                ("kc-xtc-threshold", value(cfg.kc.xtc_threshold)->default_value(0.1), "KoboldCpp ONLY. XTC threshold. (minimum: 0)")
                ("kc-xtc-probability", value(cfg.kc.xtc_probability)->default_value(0.0), "KoboldCpp ONLY. XTC probability. Set to above 0 to enable XTC. (minimum: 0)")
                ("kc-nsigma", value(cfg.kc.nsigma)->default_value(0.0), "KoboldCpp ONLY. Top N-Sigma value. Set to above 0 to enable nsigma. (minimum: 0)")
                ...
                ("sd-host", value(cfg.sd.host)->default_value("localhost"), "SD host")
                ("sd-port", value(cfg.sd.port)->default_value("7860"), "SD port")
                ("sd-prompt-file", value(cfg.sd.prompt_file)->default_value("prompt"), "SD prompt file")
                ("sd-negative-prompt-file", value(cfg.sd.negative_prompt_file)->default_value("negative_prompt"), "SD negative prompt file")
                ("sd-output-file", value(cfg.sd.output_file)->default_value("{{datetime}}.png"), "SD output PNG file")
                ...
                ("sd-styles", value(cfg.sd.styles), "SD styles")
                ("sd-seed", value(cfg.sd.seed)->default_value(-1), "SD seed")
                ("sd-subseed", value(cfg.sd.subseed)->default_value(-1), "SD subseed")
                ("sd-subseed-strength", value(cfg.sd.subseed_strength)->default_value(0), "SD subseed strength")
                ("sd-seed-resize-from-h", value(cfg.sd.seed_resize_from_h)->default_value(-1), "SD seed resize from height")
                ("sd-seed-resize-from-w", value(cfg.sd.seed_resize_from_w)->default_value(-1), "SD seed resize from width")
                ("sd-sampler-name", value(cfg.sd.sampler_name)->default_value("Euler a"), "SD sampler name")
                ("sd-scheduler", value(cfg.sd.scheduler)->default_value("Automatic"), "SD scheduler")
                ("sd-batch_size", value(cfg.sd.batch_size)->default_value(1), "SD batch size")
                ("sd-n-iter", value(cfg.sd.n_iter)->default_value(1), "SD n iter")
                ("sd-steps", value(cfg.sd.steps)->default_value(30), "SD steps")
                ("sd-cfg-scale", value(cfg.sd.cfg_scale)->default_value(7), "SD cfg scale")
                ("sd-width", value(cfg.sd.width)->default_value(1024), "SD image width")
                ("sd-height", value(cfg.sd.height)->default_value(1024), "SD image height")
                ...
                ("sd-eta", value(cfg.sd.eta)->default_value(0), "SD eta")
                ("sd-denoising-strength", value(cfg.sd.denoising_strength)->default_value(0.75, "0.75"), "SD denoising strength")
                ("sd-s-min-uncond", value(cfg.sd.s_min_uncond)->default_value(0), "SD s min uncond")
                ("sd-s-churn", value(cfg.sd.s_churn)->default_value(0), "SD s churn")
                ("sd-s-tmax", value(cfg.sd.s_tmax)->default_value(0), "SD s tmax")
                ("sd-s-tmin", value(cfg.sd.s_tmin)->default_value(0), "SD s tmin")
                ("sd-s-noise", value(cfg.sd.s_noise)->default_value(1), "SD s noise")
                ("sd-override-settings", value(cfg.sd.override_settings)->default_value(""), "SD override settings")
                ...
                ("sd-refiner-checkpoint", value(cfg.sd.refiner_checkpoint)->default_value(""), "SD refiner checkpoint")
                ("sd-refiner-switch-at", value(cfg.sd.refiner_switch_at)->default_value(0.8, "0.8"), "SD refiner switch at")
                ("sd-disable-extra-networks", po::bool_switch(&cfg.sd.disable_extra_networks)->default_value(false), "SD disable extra networks")
                ("sd-firstpass-image", value(cfg.sd.firstpass_image)->default_value(""), "SD firstpass image")
                ("sd-comments", value(cfg.sd.comments)->default_value(""), "SD comments")
                ("sd-force-task-id", value(cfg.sd.force_task_id)->default_value(""), "SD force task id")
                ("sd-sampler-index", value(cfg.sd.sampler_index)->default_value(""), "SD sampler index")
                ("sd-script-name", value(cfg.sd.script_name)->default_value(""), "SD script name")
                ("sd-script-args", value(cfg.sd.script_args), "SD script_args")
                ...
                ("sd-ad-model", value(cfg.sd.alwayson_scripts.adetailer_parametesrs.args1.ad_model)->default_value("face_yolov8n.pt"), "SD ADetailer model")
                ("sd-ad-prompt", value(cfg.sd.alwayson_scripts.adetailer_parametesrs.args1.ad_prompt)->default_value(""), "SD ADetailer prompt")
                ("sd-ad-negative-prompt", value(cfg.sd.alwayson_scripts.adetailer_parametesrs.args1.ad_negative_prompt)->default_value(""), "SD ADetailer negative prompt")
                ("sd-infotext", value(cfg.sd.infotext)->default_value(""), "SD infotext")
                ...
                ("sd-txt2img-target", value(cfg.sd.txt2img.target)->default_value("/sdapi/v1/txt2img"), "SD txt2img target")
                ("sd-enable-hr", po::bool_switch(&cfg.sd.txt2img.enable_hr)->default_value(false), "SD enable hr")
                ("sd-firstphase-width", value(cfg.sd.txt2img.firstphase_width)->default_value(0), "SD firstphase width")
                ("sd-firstphase-height", value(cfg.sd.txt2img.firstphase_height)->default_value(0), "SD firstphase height")
                ("sd-hr-scale", value(cfg.sd.txt2img.hr_scale)->default_value(0), "SD hr scale")
                ("sd-hr-upscaler", value(cfg.sd.txt2img.hr_upscaler)->default_value("SwinIR_4x"), "SD hr upscaler")
                ("sd-hr-second-pass-steps", value(cfg.sd.txt2img.hr_second_pass_steps)->default_value(0), "SD hr second pass steps")
                ("sd-hr-resize-x", value(cfg.sd.txt2img.hr_resize_x)->default_value(0), "SD hr resize x")
                ("sd-hr-resize-y", value(cfg.sd.txt2img.hr_resize_y)->default_value(0), "SD hr resize y")
                ("sd-hr-checkpoint-name", value(cfg.sd.txt2img.hr_checkpoint_name)->default_value(""), "SD hr checkpoint name")
                //("sd-hr-prompt", value(cfg.sd_txt2img_params.hr_prompt)->default_value(""), "SD hr prompt")
                //("sd-hr-negative-prompt", value(cfg.sd_txt2img_params.hr_negative_prompt)->default_value(""), "SD hr negative prompt")

                ("sd-img2img-target", value(cfg.sd.img2img.target)->default_value("/sdapi/v1/img2img"), "SD img2img target")
                ("sd-init-images", value(cfg.sd.img2img.init_images)->multitoken(), "SD img2img init_images (Base64 encoded images)")
                ("sd-seed-resize-from-h", value(cfg.sd.img2img.seed_resize_from_h)->default_value(-1), "SD img2img seed_resize_from_h")
                ("sd-seed-resize-from-w", value(cfg.sd.img2img.seed_resize_from_w)->default_value(-1), "SD img2img seed_resize_from_w")
                ("sd-resize-mode", value(cfg.sd.img2img.resize_mode)->default_value(0), "SD img2img resize_mode [0 - 3] (0: Just resize, 1: Crop and resize, 2: Resize and fill, 3: Just resize)")
                ("sd-image-cfg-scale", value(cfg.sd.img2img.image_cfg_scale)->default_value(1.0), "SD img2img image_cfg_scale")
                ("sd-mask", value(cfg.sd.img2img.mask)->default_value(""), "SD img2img mask (Base64 encoded image)")
                ("sd-mask-blur-x", value(cfg.sd.img2img.mask_blur_x)->default_value(4), "SD img2img mask_blur_x")
                ("sd-mask-blur-y", value(cfg.sd.img2img.mask_blur_y)->default_value(4), "SD img2img mask_blur_y")
                ("sd-mask-blur", value(cfg.sd.img2img.mask_blur)->default_value(4), "SD img2img mask_blur")
                ("sd-mask-round", po::bool_switch(&cfg.sd.img2img.mask_round)->default_value(true), "SD img2img mask_round")
                ("sd-inpainting-fill", value(cfg.sd.img2img.inpainting_fill)->default_value(0), "SD img2img inpainting_fill")
                ("sd-inpaint-full-res", po::bool_switch(&cfg.sd.img2img.inpaint_full_res)->default_value(true), "SD img2img inpaint_full_res")
                ("sd-inpaint-full-res-padding", value(cfg.sd.img2img.inpaint_full_res_padding)->default_value(0), "SD img2img inpaint_full_res_padding")
                ("sd-inpainting-mask-invert", value(cfg.sd.img2img.inpainting_mask_invert)->default_value(0), "SD img2img inpainting_mask_invert")
                ("sd-initial-noise-multiplier", value(cfg.sd.img2img.initial_noise_multiplier)->default_value(1.0), "SD img2img initial_noise_multiplier")
                ("sd-latent-mask", value(cfg.sd.img2img.latent_mask)->default_value(""), "SD img2img latent_mask (Base64 encoded image)")

                ("sb-host", value(cfg.sb.host)->default_value("localhost"), "SB host")
                ("sb-port", value(cfg.sb.port)->default_value("5001"), "SB port")
                ("sb-target", value(cfg.sb.target)->default_value("/voice"), "SB voide target")
                ("sb-text-file", value(cfg.sb.text_file)->default_value("text"), "SB text file")
                ("sb-output-file", value(cfg.sb.output_file)->default_value("{{datetime}}.wav"), "SB output WAV")
                ("sb-text", po::value<std::string>()->default_value("")->notifier(make_unescape_string_notifier(cfg.sb.text)), "SB text")
                ("sb-model-name", value(cfg.sb.model_name)->default_value(""), "SB model name")
                ("sb-model-id", value(cfg.sb.model_id)->default_value(0), "SB model id")
                ("sb-speaker-name", value(cfg.sb.speaker_name)->default_value(""), "SB speaker name")
                ("sb-speaker-id", value(cfg.sb.speaker_id)->default_value(0), "SB speaker id")
                ("sb-sdp-ratio", value(cfg.sb.sdp_ratio)->default_value(0.2, "0.2"), "SB sdp ratio")
                ("sb-noise", value(cfg.sb.noise)->default_value(0.6, "0.6"), "SB noise")
                ("sb-noisew", value(cfg.sb.noisew)->default_value(0.8, "0.8"), "SB noisew")
                ("sb-length", value(cfg.sb.length)->default_value(1), "SB length")
                ("sb-language", value(cfg.sb.language)->default_value(""), "SB language")
                ("sb-auto-split", po::bool_switch(&cfg.sb.auto_split)->default_value(true), "SB auto split")
                ("sb-split-interval", value(cfg.sb.split_interval)->default_value(0.5, "0.5"), "SB split interval")
                ("sb-assist-text", value(cfg.sb.assist_text)->default_value(""), "SB assist text")
                ("sb-assist-text-weight", value(cfg.sb.assist_text_weight)->default_value(1), "SB assist text weight")
                ("sb-style", value(cfg.sb.style)->default_value(""), "SB style")
                ("sb-style-weight", value(cfg.sb.style_weight)->default_value(1), "SB style weight")
                ("sb-reference-audio-path", value(cfg.sb.reference_audio_path)->default_value(""), "SB reference audio path")

                ("cu-host", value(cfg.cu.host)->default_value("localhost"), "Comfy UI host")
                ("cu-port", value(cfg.cu.port)->default_value("8188"), "Comfy UI port")
                ("cu-prompt-target", value(cfg.cu.prompt_target)->default_value("/prompt"), "Comfy UI prompt target")
                ("cu-upload-image-target", value(cfg.cu.upload_image_target)->default_value("/upload/image"), "Comfy UI upload image target")
                ("cu-prompt", po::value<std::string>()->default_value("")->notifier(make_unescape_string_notifier(cfg.cu.prompt)), "Comfy UI prompt")
                ("cu-prompt-file", value(cfg.cu.prompt_file)->default_value("prompt.json"), "Comfy UI prompt file")
                ("cu-output-directory", value(cfg.cu.output_directory)->default_value("output"), "Comfy UI output directory")
                ("cu-upload-images", value(cfg.cu.upload_images)->multitoken(), "Comfy UI upload images (macro_name=local_path)")
